    ...
    return (
        <div className="flex self-center my-5 w-full flex-col gap-4">
                    ...
                    {images.map((img: File) => {
                        return (
                            <div key={img.name} className='w-full relative my-5 mx-auto text-right animate-fadeOut'>
                                <span onClick={() => removeHandler(img)} className='absolute right-8 top-8 bg-white p-3 shadow-lg text-red-700 z-50 w-max rounded-md cursor-pointer hover:scale-110 transition-transform duration-300 ease-in-out hover:shadow-2xl'><ImCross /></span>
            ...
            <div className="flex justify-center md:items-center flex-grow my-4">
                <label htmlFor="ff" className="p-1 cursor-pointer self-end hover:animate-bounce">
