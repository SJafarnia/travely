...
async function TravelPartTemplate({ partNumber, travelId }: { partNumber: number, travelId: string }) {
    ...
    return (
        <div className='w-10/412 mx-auto text-jet'>
            <StatusBar date={`${postData?.date.getUTCDate()} - ${postData?.date.getUTCMonth()} - ${postData?.date.getUTCFullYear()}`} />
            <GalleySection images={postData?.imageGallery} mapData={postData.mapData} popUp={postData?.title} />
            <DescriptionSection description={postData?.description} tips={postData?.recommendations} />
        </div>
    )
}
